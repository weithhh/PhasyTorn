...
void fill_dac_sine_table(uint16_t *table, uint16_t samples_count, uint16_t max_value, uint16_t min_value = 0) {
	float step = 2 * M_PI / samples_count;

	for (uint16_t i  = 0; i < samples_count; i++) {
		table[i] = round((sin(i * step) + 1) * (max_value / 2) + min_value);
	}
}
