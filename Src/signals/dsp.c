...
static uint16_t circular_buffer[PK2PK_AVERAGING_WINDOW];
...
uint16_t signals_dsp_pk2pk_average(uint16_t new_value) {
	buffer_head++;
	if (buffer_head == PK2PK_AVERAGING_WINDOW) buffer_head = 0;
	circular_buffer[buffer_head] = new_value;

	uint32_t sum = 0;
	for (uint16_t i = 0; i < PK2PK_AVERAGING_WINDOW; i++) {
		sum += circular_buffer[i];
	}

	return sum / PK2PK_AVERAGING_WINDOW;
}
